...
import com.demo.network.util.IOUtils;
...
public class DefaultHttpClient {
    ...
    public HttpResponse get(URL uri, Map<String, List<String>> headers) {
        ...
        GetRequest get = new GetRequest(uri, headers);

        return getOrPost(get);
    }

    private HttpResponse getOrPost(Request request) {
        ...
        try {
            ...
            if (request instanceof PostRequest) {
                byte[] payload = ((PostRequest) request).getBodyData();
            ...
            mErrorMessage = ((request instanceof PostRequest) ? "POST " : "GET ") + ": " + e.getLocalizedMessage();
        } finally {
            if (conn != null) {
                conn.disconnect();
            }
